function resultsTest = sigmaPiSigmaPiELM(inputTrain, inputTest, outputTrain, outputTest, inputRange, hiddenNodesNo, transferFunction, datasetType)
...
outputWeightsNo = 2 ^ hiddenNodesNo;
...
inputWeights = inputRange(1) + (inputRange(2)- inputRange(1)) .* randn(hiddenNodesNo, (2 ^ id));
    ...
    for currentHiddenNodeNo = 1:hiddenNodesNo
        if strcmp(transferFunction, 'sig') || strcmp(transferFunction, 'sin') || strcmp(transferFunction, 'tribas') || strcmp(transferFunction, 'radbas') || strcmp(transferFunction, 'gaussian')
            if strcmp(transferFunction, 'sig')
                HTrain(currentSample, currentHiddenNodeNo) = logsig(sigmaPiNeuron(inputTrain(currentSample, :), inputWeights(currentHiddenNodeNo, :)));
            end
            if strcmp(transferFunction, 'sin')
                HTrain(currentSample, currentHiddenNodeNo) = sin(sigmaPiNeuron(inputTrain(currentSample, :), inputWeights(currentHiddenNodeNo, :)));
            end
            if strcmp(transferFunction, 'tribas')
                HTrain(currentSample, currentHiddenNodeNo) = tribas(sigmaPiNeuron(inputTrain(currentSample, :), inputWeights(currentHiddenNodeNo, :)));
            end
            if strcmp(transferFunction, 'radbas')
                HTrain(currentSample, currentHiddenNodeNo) = radbas(sigmaPiNeuron(inputTrain(currentSample, :), inputWeights(currentHiddenNodeNo, :)));
            end
            if strcmp(transferFunction, 'gaussian')
                HTrain(currentSample, currentHiddenNodeNo) = gaussian(sigmaPiNeuron(inputTrain(currentSample, :), inputWeights(currentHiddenNodeNo, :)));
            end
        else
...
for currentSample = 1:samplesNoTest
    for currentHiddenNodeNo = 1:hiddenNodesNo
        if strcmp(transferFunction, 'sig')
            HTest(currentSample, currentHiddenNodeNo) = logsig(sigmaPiNeuron(inputTest(currentSample, :), inputWeights(currentHiddenNodeNo, :)));
        end
        if strcmp(transferFunction, 'sin')
            HTest(currentSample, currentHiddenNodeNo) = sin(sigmaPiNeuron(inputTest(currentSample, :), inputWeights(currentHiddenNodeNo, :)));
        end
        if strcmp(transferFunction, 'tribas')
            HTest(currentSample, currentHiddenNodeNo) = tribas(sigmaPiNeuron(inputTest(currentSample, :), inputWeights(currentHiddenNodeNo, :)));
        end
        if strcmp(transferFunction, 'radbas')
            HTest(currentSample, currentHiddenNodeNo) = radbas(sigmaPiNeuron(inputTest(currentSample, :), inputWeights(currentHiddenNodeNo, :)));
        end
        if strcmp(transferFunction, 'gaussian')
            HTest(currentSample, currentHiddenNodeNo) = gaussian(sigmaPiNeuron(inputTest(currentSample, :), inputWeights(currentHiddenNodeNo, :)));
        end
    end
end
...
for currentSample = 1:samplesNoTrain
    [~, augSigmaPiHTrain(currentSample, :)] = sigmaPiNeuron(HTrain(currentSample, :), w);
end

% Create the augmented matrix for the test set
for currentSample = 1:samplesNoTest
    [~, augSigmaPiHTest(currentSample, :)] = sigmaPiNeuron(HTest(currentSample, :), w);
end
